...
Deno.serve(async req=>{try{
  ...
  const target=counts.sort((a,b)=>a.count-b.count)[0];if((active?.length??0)>=100&&target.count>=20)return json({status:'full',total:active?.length,counts});
  const needed=Math.min(20,20-target.count);if(needed<=0)return json({status:'balanced',total:active?.length,counts});
